...
class controller:
	...
	def track_point(self):
		...
		if(self.controller_type=='a'):
			...
			def distance(d_X, d_Y, c_X, c_Y):
					return math.sqrt(pow(d_X-c_X,2)+pow(d_Y - c_Y,2))
			
			def wrap_to_pi(angle):
				return (angle + math.pi) % (2*math.pi) - math.pi

			def orient_destination(c_x, c_y, c_theta, d_x, d_y):
				dx = d_x - c_x
				dy = d_y - c_y

				theta_desired = math.atan2(dy, dx)
				heading_error = wrap_to_pi(theta_desired - c_theta)

				if abs(heading_error) > 0.1:
					return 0, 2 * heading_error, False
				else:
					return 0, 0, True

			def move_to_destination(lool, c_x, c_y, d_x, d_y):
				if lool == True:
					rho = distance(d_x, d_y, c_x, c_y)
					return 20 * rho, 0, False
				else:
					return 0, 0, True
			
			def final_orientation(lool, c_theta, d_theta):
				if lool == True:
					heading_error = wrap_to_pi(d_theta - c_theta)

					if abs(heading_error) > 0.05:		
						return 0, 0.8 * heading_error, False
					else:
						return 0, 0, False
				return 0, 0, False

			#1. turn your robot to face the target position
			c_v, c_w, done1 = orient_destination(c_posX, c_posY, c_theta, d_posX, d_posY)

			#2. move your robot forward to the target position
			if done1:
				c_v, c_w, done2 = move_to_destination(True, c_posX, c_posY, d_posX, d_posY)
			else:
				done2 = False
			
			#3. turn your robot to face the target orientation
			if done2:
				c_v, c_w, done3 = final_orientation(True, c_theta, d_theta)
